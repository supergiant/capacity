...
func (h *workersHandler) getWorker(w http.ResponseWriter, r *http.Request) {
	...
	if err = json.NewEncoder(w).Encode(worker); err != nil {
		log.Errorf("handler: kubescaler: get %s worker: failed to write response: %v", machineID, err)
		w.WriteHeader(http.StatusInternalServerError)
	}
}
...
func (h *workersHandler) updateWorker(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		log.Errorf("handler: kubescaler: patch worker: reserve id=%s, %v", machineID, err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	}

	if err = json.NewEncoder(w).Encode(worker); err != nil {
		log.Errorf("handler: kubescaler: patch %s worker: failed to write response: %v", machineID, err)
		w.WriteHeader(http.StatusInternalServerError)
	}
}
...
func (h *workersHandler) deleteWorker(w http.ResponseWriter, r *http.Request) {
	...
	log.Infof("handler: kubescaler: %s worker has been deleted", machineID)
